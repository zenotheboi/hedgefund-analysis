...
pageno(s);
s.addNotes("Some of our inputs are genuine guesses - we don't know the exact borrow rate, how much of the position actually gets lent out, or the trading cost. So the fair question is: if those guesses are off, does the whole thing fall apart? That's what this slide tests. Here's how. We KEEP the model accuracy fixed - at 90%, then 80% - and keep our own choices fixed, like sizing and timing. Then we take the four things we genuinely don't know - borrow rate, utilization, the post-event fade, and trading cost - and instead of guessing once, we draw them at random over a wide range and re-run the whole strategy, 500 times. That's the table. At a 90% model, a typical run makes about $22M, and 90% of all runs land between $18M and $27M; drop to an 80% model and it's around $19.5M. We even drew the assumptions two ways - each on its own, and all going wrong together like a liquidity crunch - and even that barely moves it. The bottom line: not a single run lost money, so our cost guesses don't break the strategy. Two honest flags to close. One - you'll notice 'precision' in the headers; that's the real risk and it's the very next slide, so hold that thought. Two - this test only shuffles the COST guesses; every run still uses the actual 2016-to-2019 prices, so 'zero losses' means in a market like that one, not a promise for a future crash.");
